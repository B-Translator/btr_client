...
* Advantages of features
  ...
  3. Make the application structured (as opposed to monolithic).  For
     example simplenews is offered as a feature and can be installed
...
* Creating features
  ...
  /bcl_captcha/, /bcl_simplenews/, /bcl_mass_contact/, etc.  In order to
  recreate them easily, the script =create-features.sh= can be used. It
  will create a feature for each list of components that is on the
...
* Making features configurable
  ...
  like disqus, recaptcha, etc. Usually they need some API keys or any
  other private attributes that are different from site to site. We
  cannot save these private attributes on the feature, because they
  are specific for each different site. But we would like to offer the
